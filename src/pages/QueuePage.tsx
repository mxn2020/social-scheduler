...
export default function QueuePage() {
    return (<div style={{ padding: "var(--space-8)", maxWidth: 1000 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            {queue.map(q => (<div key={q.id} className="card" style={{ display: "flex", alignItems: "stretch", overflow: "hidden", borderLeft: `4px solid ${q.color}` }}>
                ...
                <div style={{ padding: "var(--space-5)", flex: 1, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <p style={{ fontSize: "16px", color: "var(--color-text-primary)", lineHeight: 1.6 }}>{q.text}</p>
                    <div style={{ display: "flex", alignItems: "center", gap: "var(--space-4)" }}>
                        ...
                        <button className="btn btn-ghost btn-sm" style={{ padding: 4 }}><MoreVertical size={18} /></button>
                    </div>
                </div>
            </div>))}
        </div>
